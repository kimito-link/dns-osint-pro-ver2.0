...
このファイルは、サジェスト汚染チェッカーの重要な変更を記録します。
...
### ✨ 新機能
- **逆SEO・ネガティブサイトチェック**:
  - 5ch、爆サイ、jpnumber、ホスラブへのワンクリック検索リンクを追加
  - 個人名・会社名でネガティブな投稿が表示されていないか確認可能
  - 口コミセクションの直後に表示

### 🎨 UI/UX改善
- **リブランディング**:
  - ツール名を「サジェスト汚染チェッカー」に変更
  - ヘッダー・manifest・プライバシーポリシーを更新
- **ロゴ変更**:
  - ヘッダー・ファビコンをリバースハックの鮫ロゴ（rev.png）に統一
- **風評関係の表示順**:
  - 風評被害チェック・個人名ネガティブチェックを最優先表示に変更
- **口コミセクション**:
  - リバースハック相談（口コミ）リンクを追加（lin.ee/8XbiSq6）
  - トンマナ統一（緑ボタン・鮫ロゴ・@rephバッジ）

### 📝 技術的な変更
- `ui-components.js`: createReverseSeoButtons, createReverseSeoSection を追加
- `popup.js`: 逆SEOセクションの呼び出しを追加
