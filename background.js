...
async function analyzeSiteHealth(domain) {
  try {
    ...
    if (phpVersion) {
      ...
      // PHPバージョンチェック（8.0以上を推奨、7.4以下は警告）
      if (phpMajor < 7.4) {
        issues.push(`PHPのバージョンが非常に古いです (${phpVersion})。セキュリティリスクが極めて高く、サポートも終了しています。PHP 8.1以上への移行が必須です。`);
      } else if (phpMajor < 8.0) {
        warnings.push(`PHPのバージョンが古いです (${phpVersion})。PHP 8.1以上への更新を推奨します。`);
      } else {
        goodPoints.push(`PHPは比較的新しいバージョンです (${phpVersion})`);
      }
    } else {
      // PHPバージョンが検出できなかった場合
 ...
 * @returns {Array} 抽出された個人名の配列 [{name: string, title: string, context: string}]
 */
function extractPersonNames(html) {
  const results = [];
  const seen = new Set();
  ...
  // 🎯 役職パターン：社長・代表取締役・CEOのみに絞り込み
  const titlePatterns = [
    // 最高経営層のみ（社長・代表取締役・CEO）
    { pattern: /(代表取締役|代表|社長|会長|CEO)[:：\s　]*([ぁ-ん一-龠]{2,4})/, role: '経営層' }
  ];
  ...
  for (const {pattern, role} of titlePatterns) {
    ...
    for (const match of matches) {
      ...
      // 明らかに名前ではないものを除外
      if (name.length < 2 || name.match(/^[0-9]+$/)) continue;
  ...
  // スタッフ紹介セクションから追加抽出
  const staffSectionPattern = /(スタッフ紹介|メンバー紹介|チーム紹介|当院のスタッフ|スタッフ一覧)[\s\S]{0,500}/gi;
  const staffSections = text.match(staffSectionPattern);
  
  if (staffSections) {
    for (const section of staffSections) {
      // 2-4文字の漢字・ひらがなの名前を抽出
      const nameMatches = section.matchAll(/([ぁ-ん一-龠]{2,4})[\s　]*(?:さん|先生|氏)?/g);
      for (const match of nameMatches) {
        const name = match[1];
        if (seen.has(name)) continue;
        if (name.match(/^(当院|スタッフ|メンバー|紹介|一覧|ページ)$/)) continue;
  ...
  console.log(`個人名抽出完了: ${results.length}件`);
  return results;
}
 ...
 */
async function checkPersonReputations(domain, url) {
  try {
    console.log('=== 個人名ネガティブチェック開始 ===');
    ...
    // 個人名を抽出
    const persons = extractPersonNames(html);
    console.log('抽出された個人名:', persons);
    ...
    if (persons.length === 0) {
      return {
        ...
        persons: [],
        hasNegative: false,
        message: '個人名が検出されませんでした'
      };
    }
    
    // ネガティブキーワード（重要なものだけに絞る）
    const negativeKeywords = [
      '逮捕', '事件', '不祥事', '詐欺', 'パワハラ', 'セクハラ'
    ];
    
    // 🎯 経営層を優先的にチェック（最大3人まで、高速化のため）
    const checkResults = [];
    const executivePersons = persons.filter(p => p.role === '経営層');
    const otherPersons = persons.filter(p => p.role !== '経営層');
    const personsToCheck = [...executivePersons, ...otherPersons].slice(0, 3);
    
    console.log(`チェック対象: 経営層${executivePersons.length}名、その他${otherPersons.length}名 → 最大3名をチェック`);
    
    for (const person of personsToCheck) {
      console.log(`チェック中: ${person.name} (${person.title})`);
      ...
      // 各ネガティブキーワードでサジェストチェック
      for (const keyword of negativeKeywords) {
        const query = `${person.name} ${keyword}`;
        
        try {
          const suggests = await fetchGoogleSuggest(query);
          
          // サジェストに該当キーワードが含まれているかチェック
          for (const suggest of suggests) {
            if (suggest.includes(person.name) && suggest.includes(keyword)) {
              personResult.hasNegative = true;
              personResult.negativeSuggests.push({
                keyword,
                suggest
              });
              console.log(`⚠️ ネガティブ検出: ${suggest}`);
            }
          }
        } catch (e) {
          console.warn(`サジェスト取得エラー (${query}):`, e.message);
        }
      ...
      checkResults.push(personResult);
    }
    
    // ネガティブが見つかった人物
    const negativePersons = checkResults.filter(p => p.hasNegative);
    
    return {
      success: true,
      persons: checkResults,
      hasNegative: negativePersons.length > 0,
      negativeCount: negativePersons.length,
      totalChecked: checkResults.length
    };
    
  } catch (error) {
    return createErrorResponse(error, 'checkPersonReputations');
  }
}
