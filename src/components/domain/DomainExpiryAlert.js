/**
 * 🎨 DomainExpiryAlert Component
 * ドメイン有効期限切れ警告カードを生成する業務コンポーネント
 * @version 1.0.0
 */
 ...
 * @param {string} expirationDate - 有効期限の日付文字列
 * @param {string} domain - ドメイン名
 * @returns {string} HTML文字列
 */
function createDomainExpiryAlert(daysUntilExpiry, expirationDate, domain) {
  ...
  // 有効期限の日付をフォーマット
  const expiryDate = expirationDate ? new Date(expirationDate).toLocaleDateString('ja-JP', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  }) : '';
  ...
  if (daysUntilExpiry < 0) {
    ...
    message = `ドメインが${daysUntilExpiry}日後に切れます！`;
    urgencyMessage = '緊急の更新が必要です';
  } else if (daysUntilExpiry <= 14) {
    message = `ドメインが${daysUntilExpiry}日後に切れます！`;
    urgencyMessage = '早急な更新が必要です';
  } else {
    message = `ドメインが${daysUntilExpiry}日後に切れます！`;
    urgencyMessage = '更新の準備を始めましょう';
  }
    ...
    <div style="background: linear-gradient(135deg, #e53935 0%, #d32f2f 100%); border: 3px solid #b71c1c; padding: 20px; border-radius: 12px; box-shadow: 0 6px 12px rgba(0,0,0,0.15); margin-bottom: 20px;">
      ...
      <div style="background: rgba(255,255,255,0.95); padding: 15px; border-radius: 8px; margin-bottom: 15px;">
        ...
        <div style="color: #333; font-size: 0.95em; line-height: 1.8; margin-top: 10px;">
          ❌ <strong style="color: #d32f2f;">サイトが完全に使えなくなる</strong><br>
          ${daysUntilExpiry < 0 ? '❌ <strong>既にドメインが切れています</strong><br>' : ''}
          ❌ <strong>メールが届かなくなる</strong><br>
          ❌ <strong>顧客がアクセスできなくなる</strong><br>
          ❌ <strong>SEOランキングが失われる</strong><br>
        ...
        ${domain ? `<div style="margin-top: 5px; padding: 8px; background: #fff3e0; border-left: 3px solid #ff9800; border-radius: 4px;"><strong>対象ドメイン:</strong> ${domain}</div>` : ''}
      </div>
      ...
      <div style="display: flex; align-items: start; gap: 12px; margin-bottom: 15px;">
        ...
        <div style="flex: 1;">
          <div style="background: #fff; padding: 12px; border-radius: 8px; position: relative; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            ...
            <span style="color: #333; font-size: 0.95em; line-height: 1.6;">「これは本当に危ないよ！ドメインが切れるとサイトが完全に使えなくなっちゃう！りんくが頼りにしているリバースハックに相談してみて！ドメイン更新を安全にやってくれるよ！」</span>
          </div>
        </div>
      </div>
...
window.OsintUIComponents.createDomainExpiryAlert = createDomainExpiryAlert;
